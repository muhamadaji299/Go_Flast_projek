...
<div class="flex justify-between items-center mb-6">
  ...
  <div class="bg-white p-6 rounded shadow">
    <div class="flex justify-between items-center mb-4 flex-wrap gap-4">
      <h2 class="text-xl font-semibold text-gray-800">Table Driver</h2>
      ...
      <div class="flex gap-2 flex-wrap">
        ...
        <!-- Add Button -->
        <a href="{{ route('drivers.create')}}" class="bg-orange-500 text-white px-4 py-2 rounded hover:bg-orange-600">
      ...
      <tbody>
        ...
        <tr class="text-gray-600">
          ...
          <td class="py-2 px-4 border-b border-gray-200">
            <img src="{{ asset('storage/' . $driver->photo) }}" class="w-12 h-12 rounded-full object-cover" alt="{{ $driver->name }}">
          </td>
          <td class="py-2 px-4 border-b border-gray-200">{{ $driver->name }}</td>
          <td class="py-2 px-4 border-b border-gray-200">{{ $driver->location }}</td>
             <td class="py-2 px-4 border-b border-gray-200">{{ $driver->telepon }}</td>
            <td class="py-2 px-4 border-b border-gray-200">{{ $driver->email }}</td>
         
          <td class="py-2 px-4 border-b border-gray-200 capitalize">{{ $driver->vehicle_type }}</td>
          <td class="py-2 px-4 border-b border-gray-200">{{ $driver->description}}</td>
          <td class="py-2 px-4 border-b border-gray-200 space-x-2">
            <a href="{{ route('drivers.edit', $driver->id) }}" class="text-blue-600 hover:underline">Edit</a>
            <form action="{{ route('drivers.destroy', $driver->id) }}" method="POST" class="inline delete-form">
              @csrf
              @method('DELETE')
        ...
        @empty
        <tr>
          <td colspan="5" class="text-center py-4 text-gray-500">Data driver tidak ditemukan.</td>
        </tr>
        @endforelse
      </tbody>
    </table>
